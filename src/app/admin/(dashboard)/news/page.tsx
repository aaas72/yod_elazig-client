...
export default function AdminNewsPage() {
  ...
  const entityFolderRef = useRef<string>('news');
  ...
  const loadData = useCallback(async () => {
    try {
      setLoading(true);
      const data = await newsService.getAll({ limit: 1000, search });
      setNews(data?.news || []);
    } catch (err) {
  ...
  const openCreate = () => {
    ...
    entityFolderRef.current = `news/new-${Date.now()}`;
    setModalOpen(true);
  };
  ...
  const openEdit = (item: NewsItem) => {
    setEditingItem(item);
    setActiveLang('ar');
    entityFolderRef.current = `news/${item._id}`;
    // Extract translations from item
    const t = (item as any).translations as Record<Lang, LangFields> | undefined;
    const tagsToStr = (tags: any) => Array.isArray(tags) ? tags.join(', ') : (tags || '');
    if (t && t.ar) {
      setTranslations({
        ar: { title: t.ar?.title || '', content: t.ar?.content || '', summary: t.ar?.summary || '', category: t.ar?.category || item.category || '', tags: tagsToStr(t.ar?.tags) || item.tags?.join(', ') || '' },
        en: { title: t.en?.title || '', content: t.en?.content || '', summary: t.en?.summary || '', category: t.en?.category || '', tags: tagsToStr(t.en?.tags) },
        tr: { title: t.tr?.title || '', content: t.tr?.content || '', summary: t.tr?.summary || '', category: t.tr?.category || '', tags: tagsToStr(t.tr?.tags) },
      });
    } else {
