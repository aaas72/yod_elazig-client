...
const LANGS: { key: Lang; label: string; dir: 'rtl' | 'ltr'; flag: string; placeholders: { title: string; description: string } }[] = [
  { key: 'ar', label: 'العربية', dir: 'rtl', flag: '🇾🇪', placeholders: { title: 'أدخل عنوان البرنامج...', description: 'اكتب وصف البرنامج...' } },
  { key: 'tr', label: 'Türkçe', dir: 'ltr', flag: '🇹🇷', placeholders: { title: 'Program başlığını girin...', description: 'Program açıklamasını yazın...' } },
  { key: 'en', label: 'English', dir: 'ltr', flag: '🇬🇧', placeholders: { title: 'Enter program title...', description: 'Write program description...' } },
];
...
export default function AdminProgramsPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      setLoading(true);
      const data = await programsService.getAll({ limit: 1000, search });
      const items = Array.isArray(data?.data) ? data.data : Array.isArray(data) ? data : [];
      setProgramsData(items);
    } catch { toast.error('فشل تحميل البرامج'); } finally { setLoading(false); }
  }, [search]);
  ...
  const [sharedData, setSharedData] = useState({ startDate: '', endDate: '', location: '', category: '', coverImage: '', isPublished: true });

  const [uploading, setUploading] = useState(false);
  const coverInputRef = useRef<HTMLInputElement>(null);
  const entityFolderRef = useRef<string>('programs');
  ...
  const openCreate = () => {
    ...
    setSharedData({ startDate: '', endDate: '', location: '', category: '', coverImage: '', isPublished: true });
    entityFolderRef.current = `programs/new-${Date.now()}`;
    setModalOpen(true);
  };
  ...
  const openEdit = (item: ProgramItem) => {
    setEditingItem(item);
    setActiveLang('ar');
    entityFolderRef.current = `programs/${item._id}`;
    setTranslations({
  ...
  const handleDelete = async (id: string) => { if (!confirm('هل أنت متأكد؟')) return; try { await programsService.delete(id); toast.success('تم الحذف'); loadData(); } catch { toast.error('فشل الحذف'); } };
  const handleToggle = async (id: string) => { try { await programsService.togglePublish(id); toast.success('تم التحديث'); loadData(); } catch { toast.error('فشل التحديث'); } };

  const statusLabel = (status: string) => {
    const map: Record<string, { cls: string; label: string }> = {
      upcoming: { cls: 'bg-gray-100 text-gray-600', label: 'قادم' },
        ...
        <div className="space-y-5">
          ...
          <div>
            ...
            <RichTextEditor
              ...
              placeholder={LANGS.find(l => l.key === activeLang)?.placeholders.description || 'اكتب وصف البرنامج...'}
              dir={LANGS.find(l => l.key === activeLang)?.dir || 'rtl'}
            />
          </div>
          {/* الحقول المشتركة */}
