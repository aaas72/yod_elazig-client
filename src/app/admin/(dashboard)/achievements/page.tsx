...
export default function AdminAchievementsPage() {
  ...
  const loadData = useCallback(async () => {
    try { setLoading(true); const data = await achievementsService.getAll({ limit: 1000, search }); setItems(data?.data || []); } catch { toast.error('فشل التحميل'); } finally { setLoading(false); }
  }, [search]);
  ...
  return (
    <>
      <AdminDataTable title="" data={items} columns={columns} loading={loading} search={search} onSearchChange={setSearch} onAdd={openCreate} addLabel="إنجاز جديد" actions={(item) => (
