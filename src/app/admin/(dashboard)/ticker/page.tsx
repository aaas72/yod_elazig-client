...
export default function AdminTickerPage() {
  ...
  const entityFolderRef = useRef<string>('ticker');
  ...
  const openCreate = () => {
    ...
    entityFolderRef.current = `ticker/new-${Date.now()}`;
    setModalOpen(true);
  };

  const openEdit = (item: TickerItem) => {
    setEditingItem(item);
    entityFolderRef.current = `ticker/${item._id}`;
    setFormData({
      textAr: item.text.ar, textEn: item.text.en, textTr: item.text.tr,
      link: item.url || item.link || '', image: item.image || '', isActive: item.isActive ?? true, order: String(item.order ?? 0),
    });
    setModalOpen(true);
  };
  ...
  const handleSave = async () => {
    try {
      ...
      const payload = {
        ...
        url: formData.link, image: formData.image, isActive: formData.isActive, order: Number(formData.order),
      };
